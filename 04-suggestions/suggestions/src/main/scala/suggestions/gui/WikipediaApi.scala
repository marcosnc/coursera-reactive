...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = Observable.create( anObserver => {
       obs.subscribe(
         t => anObserver.onNext(Success(t)),
         t => {anObserver.onNext(Failure(t)); anObserver.onCompleted()},
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      ...
      obs.filter(_ => (System.currentTimeMillis()-t0)/1000 < totalSec)
    }
