...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.sanitized.concatRecovered( t => Observable.from(wikipediaSuggestion(t)).timedOut(3))

    // TO IMPLEMENT
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe { x: Try[List[String]] => x match {
      case Failure(error) => suggestionList.listData = Seq(error.getMessage)
      case Success(values: List[String]) => suggestionList.listData = values
    }}
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered( t => Observable.from(wikipediaPage(t)).timedOut(3))

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x: Try[String] => x match {
        case Failure(error) => editorpane.text = error.getMessage
        case Success(value: String) => editorpane.text = value
      }
    }
