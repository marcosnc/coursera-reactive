...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] =
      Observable.create {
        aSubscriber => {
          val aReaction: Reaction = { case ValueChanged(aTextField) => aSubscriber.onNext(aTextField.text) }
          field.subscribe(aReaction)
          Subscription(field.unsubscribe(aReaction))
        }
      }
  }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] =
      Observable.create {
        aSubscriber => {
          val aReaction: Reaction = { case ButtonClicked(aButton) => aSubscriber.onNext(aButton) }
          button.subscribe(aReaction)
          Subscription(button.unsubscribe(aReaction))
        }
      }
  }
