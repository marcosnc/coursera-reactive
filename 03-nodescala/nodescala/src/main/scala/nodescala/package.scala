...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    /** Creates a cancellable context for an execution and runs it.
      * TODO: revisar si funciona. Probar con async/await
     */
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
//    def now: T = if (f.isCompleted) f.value.get.get else throw NoSuchElementException
    def now: T = {
      try {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete {
        case Success(value) => try { p.success(cont(f)) } catch { case NonFatal(t) => p.failure(t)}
        case Failure(failure) => p.failure(failure)
      }
      p.future
    }
