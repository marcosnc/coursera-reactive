...
object Main {
  ...
  {
    val u = Future.userInput("type something: ")
    u onSuccess {
      case a => println(s"onSuccess: $a")
    }
    u onFailure {
      case a => println(s"onFailure: $a")
    }
    ...
    def main (args: Array[String])
    {
      Future.userInput("Hit ENTER to cancel... ") continueWith { _ => "You entered... " }
//      Future.userInput("Hit ENTER to cancel... ") continueWith { f => "You entered... " + f.now }
