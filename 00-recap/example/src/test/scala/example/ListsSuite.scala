...
class ListsSuite extends FunSuite {
  ...
  test("sum of Nilt") { assert(sum(Nil) === 0) }

  test("max of an empty list") { intercept[NoSuchElementException] { max(List()) } }
  test("max of Nil") { intercept[NoSuchElementException] { max(Nil) } }
